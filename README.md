...
- 「管理内容」浏览和搜索全部条目，查看详情，新增、编辑、确认删除；内置示例和导入内容均可直接编辑，修改保存在本机。
...
3. 新增组件默认 2×2，可拉成横向 4×1（1 行 4 列）；实际格数由桌面网格决定。窄卡片隐藏日期与底栏，扁卡片优先显示正文，点击正文查看全文。
4. 「组件字号」支持 14～24，默认 20；空间不足时自动缩小并省略长内容。已有组件保留原尺寸，可长按调整或重新添加。
5. 点击「导入 JSON」选择自己的文件，确认预览；通过「切换句库」选择桌面内容。
...
收藏页支持卡片预览、搜索、全文详情与一键取消收藏，可切换查看不同句库。
